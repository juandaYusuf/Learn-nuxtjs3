<template>
  <header>
    <nav class="w-full h-[64px] bg-slate-100/75 border-b shadow-md flex justify-around items-center">
      <div class="w-[40px]">
      ...
      <div class="w-[40px]">
        <UButton icon="i-heroicons-light-bulb" size="sm" color="primary" square variant="ghost" @click='toggleSlider' />
      </div>
    </nav>
    <AppContainerSlider class="absolute top-[75px] right-[7%] -z-10"
      :class="isSliderOpen ? '-translate-y-[0%] ease-in-out duration-500' : '-translate-y-[150%] ease-in-out duration-500'" />
  </header>
</template>
